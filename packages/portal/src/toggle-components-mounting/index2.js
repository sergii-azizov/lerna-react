...
import { lazyLoad } from '../helpers.js';
...
export default class ToggleComponentsMounting2 extends Component {
    ...
    componentDidMount() {
        setTimeout(() => { this.setState({ Icon1: lazyLoad('icon') }); }, 1000);
        setTimeout(() => { this.setState({ Icon1: () => <div>ICON</div> }); }, 2000);
        setTimeout(() => { this.setState({ Icon1: lazyLoad('icon') }); }, 3000);
        setTimeout(() => { this.setState({ Icon1: () => <div>ICON</div> }); }, 4000);
        setTimeout(() => { this.setState({ Icon1: lazyLoad('icon') }); }, 5000);

        setTimeout(() => { this.setState({ Button: lazyLoad('button') }); }, 1000);
        setTimeout(() => { this.setState({ Button: () => <div>Button</div> }); }, 2000);
        setTimeout(() => { this.setState({ Button: lazyLoad('button') }); }, 3000);
    }
